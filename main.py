...
# In-memory high score for browser compatibility
current_high_score = 0

# Load high score
async def load_high_score():
    global current_high_score
    try:
        if os.path.exists(HIGH_SCORE_FILE):
            with open(HIGH_SCORE_FILE, "r") as file:
                current_high_score = json.load(file)
    except:
        # In browser environment, use the in-memory score
        pass
    return current_high_score

# Save high score
async def save_high_score(score):
    global current_high_score
    ...
    try:
        ...
        # In browser environment, keep using the in-memory score
        pass
...
async def home_screen(high_score):
    running = True
    while running:
        screen.fill(BLACK)
...
async def game_over_screen(score, high_score):
    running = True
    while running:
        screen.fill(BLACK)
...
# Main game function
async def game_loop(platforms, difficulty):
    # Player settings
    ...
    # Load high score
    high_score = await load_high_score()
    ...
    while running:
        ...
        if game_started:
            ...
            if player_y > SCREEN_HEIGHT and not invincible:
                ...
                if score > high_score:
                    high_score = score
                    await save_high_score(high_score)
                restart = await game_over_screen(score, high_score)
                if restart:  # Restart the game
                    return True
                else:  # Exit the game
                    running = False
                    return False
...
async def main():
    ...
    high_score = await load_high_score()  # Load high score at start

    running = True
    while running:
        if current_screen == "home":
            result = await home_screen(high_score)
            if result == "start":
                current_screen = "game"
            ...
            result = await game_loop(platforms, selected_difficulty)
            if result:
                current_screen = "home"
            else:
                running = False
